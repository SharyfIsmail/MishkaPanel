...
public class FtdiSerialDriver implements UsbSerialDriver {
    ...
    public FtdiSerialDriver(UsbDevice device) {
        ...
        for( int port = 0; port < device.getInterfaceCount(); port++) {
            mPorts.add((UsbSerialPort) new FtdiSerialPort(mDevice, port));
        }
    }
    ...
    public class FtdiSerialPort extends CommonUsbSerialPort {
        ...
        private void setBaudrate(int baudRate) throws IOException {
            ...
            android.util.Log.d(TAG, String.format("baud rate=%d, effective=%d, error=%.1f%%, value=0x%04x, index=0x%04x, divisor=%d, subdivisor=%d",
