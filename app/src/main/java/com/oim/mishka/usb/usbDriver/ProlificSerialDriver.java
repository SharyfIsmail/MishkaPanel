                ...
                synchronized (mReadStatusThreadLock) {
                    if (mReadStatusThread == null) {
                        ...
                        if (readBytes != STATUS_BUFFER_SIZE) {
                            android.util.Log.w(TAG, "Could not read initial CTS / DSR / CD / RI status");
                        } else {
                            mStatus = buffer[STATUS_BYTE_IDX] & 0xff;
                        }
            ...
            if (mDevice.getDeviceClass() == 0x02) {
                ...
                try {
                    ...
                    if (maxPacketSize0 == 64) {
                        ...
                        android.util.Log.w(TAG, "Could not detect PL2303 subtype, "
                                + "Assuming that it is a HX device");
                        mDeviceType = DEVICE_TYPE_HX;
                    }
                } catch (NoSuchMethodException e) {
                    android.util.Log.w(TAG, "Method UsbDeviceConnection.getRawDescriptors, "
                            + "required for PL2303 subtype detection, not "
                            + "available! Assuming that it is a HX device");
                    mDeviceType = DEVICE_TYPE_HX;
                } catch (Exception e) {
                    android.util.Log.e(TAG, "An unexpected exception occured while trying "
                ...
                synchronized (mReadStatusThreadLock) {
                    if (mReadStatusThread != null) {
                        try {
                            ...
                            android.util.Log.w(TAG, "An error occured while waiting for status read thread", e);
                        }
                    }
                }
                resetDevice();
